...
Version: 0.2.0
(与 bbt_mcp 同源，用于 hzr_mcp 接入小智 MCP)

Usage (env):
    export MCP_ENDPOINT=<ws_endpoint>   # full URL with token
    # Or separately:
    export MCP_BASE_URL=wss://api.xiaozhi.me/mcp/
    export MCP_TOKEN=<your_jwt_token>

Usage (CLI):
    python mcp_pipe.py hzr_mcp.py              # use token from .env
    python mcp_pipe.py hzr_mcp.py -t <token>   # override token via --token

Config: $MCP_CONFIG or ./mcp_config.json
"""
...
# Default base URL for Xiaozhi MCP (without token)
MCP_BASE_URL_DEFAULT = "wss://api.xiaozhi.me/mcp/"


def resolve_endpoint_url(token_override: str | None) -> str:
    """Resolve final MCP WebSocket URL.

    Priority:
    1. MCP_ENDPOINT (full URL) + token_override/MCP_TOKEN replaces ?token= if provided
    2. MCP_ENDPOINT alone if it already contains token and no override
    3. MCP_BASE_URL + (token_override or MCP_TOKEN)
    """
    token = token_override or os.environ.get("MCP_TOKEN")
    endpoint = os.environ.get("MCP_ENDPOINT", "").strip()
    base_url = os.environ.get("MCP_BASE_URL", MCP_BASE_URL_DEFAULT).strip().rstrip("/")
    ...
    if not token:
        logger.error(
            "Please set MCP_ENDPOINT (full URL) or (MCP_BASE_URL + MCP_TOKEN/--token)"
        )
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MCP stdio <-> WebSocket pipe (小智 MCP 接入)")
    parser.add_argument(
        "-t", "--token",
        metavar="TOKEN",
        help="MCP endpoint token (overrides MCP_TOKEN env)",
    )
    parser.add_argument(
        "target",
        nargs="?",
        help="Server name from config or path to server script (e.g. hzr_mcp.py)",
    )
    args, _ = parser.parse_known_args()

    endpoint_url = resolve_endpoint_url(args.token)
