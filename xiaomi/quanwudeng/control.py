#!/usr/bin/env python3
"""全屋灯开关控制。供 hzr_mcp.py 通过 subprocess 调用。

用法:
  python control.py on   # 打开全屋灯
  python control.py off  # 关闭全屋灯

从 ../devices.json 读取 全屋灯 的 ip/token；使用 MIOT set_property(siid=2, piid=1)。
"""
...
def main() -> int:
    ...
    on = action == "on"
    try:
        dev = load_device_by_name("全屋灯")
        ip = dev.get("ip")
        token = dev.get("token")
        if not ip or not token:
            print("全屋灯 缺少 ip 或 token", file=sys.stderr)
            return 1
