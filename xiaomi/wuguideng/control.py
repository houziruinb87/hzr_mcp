#!/usr/bin/env python3
"""乌龟灯开关控制。供 hzr_mcp.py 通过 subprocess 调用。

用法:
  python control.py on   # 打开乌龟灯
  python control.py off # 关闭乌龟灯

从 ../devices.json 读取 乌龟灯 的 ip/token；使用 MIOT set_property(siid=2, piid=1)。
"""
...
def set_power(ip: str, token: str, on: bool) -> None:
    ...
    # 与 hzr_mcp 同环境时 miiocli 在同目录（venv/bin）
...
def main() -> int:
    ...
    on = action == "on"
    try:
        dev = load_device_by_name("乌龟灯")
        ip = dev.get("ip")
        token = dev.get("token")
        if not ip or not token:
            print("乌龟灯 缺少 ip 或 token", file=sys.stderr)
            return 1
