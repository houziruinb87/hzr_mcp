...
**一键脚本（本机与 NAS 同网时）**：在仓库根目录执行 `bash docs/deploy_migpt_next_on_nas.sh`，会在 NAS 的 `/data_n003/data/udata/real/18510411307/docker/MiGPT-Next` 下创建目录、上传 `config.js` 与 `docker-compose.yml`，并用 **Docker Compose** 启动。该目录下会有：
- `config.js`：你的配置（小米账号、设备名、onMessage 等）
- `docker-compose.yml`：服务定义，便于统一管理、迁移

修改配置后重启：`ssh jkj "cd /data_n003/data/udata/real/18510411307/docker/MiGPT-Next && sg docker -c 'docker compose restart'"`。
