...
**Docker Compose 方式（推荐）**：在 NAS 的 `docker/xiao_music` 目录下已有 `music`、`conf` 子目录时，将本仓库 `docs/xiao_music-docker-compose.yml` 复制到该目录并重命名为 `docker-compose.yml`，然后执行 `docker compose up -d`。目录内将包含 `docker-compose.yml`、`music`、`conf`，便于统一管理与迁移。修改配置或音乐后重启：`docker compose restart`。
...
**语音不走 xiao_music（还是播 QQ 音乐/自带源）时**：① 必须说 **「播放歌曲」+ 歌名**（如「小爱同学，播放歌曲 誓要入刀山」），只说「播放 xxx」不会走 XiaoMusic；② 在 Web 设置/主页打开 **【语音口令】或【获取对话记录】** 开关；③ 确认已在设置里勾选你的小爱设备；④ 首次保存设备后重启一次容器。歌需在 music 目录内或开启「没有就下载」。
...
**一键脚本（本机与 NAS 同网时）**：在仓库根目录执行 `bash docs/deploy_migpt_next_on_nas.sh`，会在 NAS 的 `/path/to/nas/udata/real/YOUR_USER_ID/docker/MiGPT-Next` 下创建目录、上传 `config.js` 与 `docker-compose.yml`，并用 **Docker Compose** 启动。该目录下会有：
- `config.js`：你的配置（小米账号、设备名、onMessage 等）
- `docker-compose.yml`：服务定义，便于统一管理、迁移

修改配置后重启：`ssh jkj "cd /path/to/nas/udata/real/YOUR_USER_ID/docker/MiGPT-Next && sg docker -c 'docker compose restart'"`。
...
5. 使用 Docker Compose 启动（推荐）：将本仓库 `docs/migpt-next-docker-compose.yml` 复制到与 `config.js` 同目录并重命名为 `docker-compose.yml`，然后执行：
   ```bash
   docker compose up -d
   ```
   或单次运行：`docker run -it --rm -v $(pwd)/config.js:/app/config.js idootop/migpt-next:latest`。  
   若在极空间用 Docker 图形界面：创建容器时挂载 `config.js` 到 `/app/config.js`，镜像可用 `docker.1ms.run/idootop/migpt-next:latest`（国内拉取更稳）。
...
# 第三步（可选）：Open-XiaoAI Server（小智同学唤醒）

用于**双唤醒**：说「小智同学」走本地/小智云逻辑，说「小爱同学」仍走原小米逻辑。需先给音箱刷 Open-XiaoAI 固件并指向 NAS。

- **NAS 部署路径**：`/path/to/nas/udata/real/YOUR_USER_ID/docker/Open-XiaoAI-Server`
- **端口**：4399
- **配置**：`config.py` 中 `wakeup.keywords = ["小智同学"]`，`xiaozhi.WEBSOCKET_URL` 先填小智云体验 `wss://api.tenclass.net/xiaozhi/v1/`
- **Compose**：本仓库 `docs/open-xiaoai-server-docker-compose.yml`、`docs/open-xiaoai-server-config.py` 已就绪；NAS 上复制为 `docker-compose.yml` 与 `config.py` 后执行 `docker compose up -d`
- 重启：`cd Open-XiaoAI-Server && sg docker -c 'docker compose restart'`
...
| 第三步（可选） | **Open-XiaoAI Server** | 「小智同学」唤醒 → 小智云/本地后端；「小爱同学」保持原小米逻辑 |
