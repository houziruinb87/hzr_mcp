...
def _mask_bearer(token: str) -> str:
    if not token:
        return ""
    token = token.strip()
...
class WebSocketServer:
    ...
    async def _handle_connection(self, websocket: websockets.ServerConnection):
        ...
        try:
            ...
            try:
                hdrs = websocket.request.headers
                device_id = hdrs.get("device-id", None)
                client_id = hdrs.get("client-id", None)
                ...
                self.logger.bind(tag=TAG).warning(
                    "[小智] WS认证失败 "
                    f"ip={remote_ip} path={websocket.request.path} "
                    f"device-id={device_id} client-id={client_id} "
                    f"authorization={_mask_bearer(auth)!r} err={str(e)}"
                )
            except Exception as log_e:
                self.logger.bind(tag=TAG).warning(f"[小智] WS认证失败（记录日志时异常）: {log_e}")
    ...
    async def _handle_auth(self, websocket: websockets.ServerConnection):
        # 先认证，后建立连接
        if self.auth_enable:
            # 注意：websockets 的 Headers 是大小写不敏感的，这里不要转 dict 以免丢失能力
