...
class ConnectionHandler:
    ...
    async def handle_connection(self, ws: websockets.ServerConnection):
        try:
            self.logger.bind(tag=TAG).info("[小智] ========== 新 WebSocket 连接 ==========")
            ...
            self.logger.bind(tag=TAG).info(
                f"[小智] 连接详情 IP={self.client_ip} device-id={self.device_id} path={request_path}"
            )
            ...
            try:
                ...
                self.logger.bind(tag=TAG).info(
                    f"[小智] 客户端断开连接 IP={self.client_ip} device-id={self.device_id}"
                )
    ...
    async def _route_message(self, message):
        ...
        if isinstance(message, str):
            self.logger.bind(tag=TAG).info(
                f"[小智] 收到文本消息 len={len(message)} 内容前100字: {message[:100]!r}"
            )
        elif isinstance(message, bytes):
            self.logger.bind(tag=TAG).info(
                f"[小智] 收到音频消息 len={len(message)} bytes"
            )
        # 检查是否已经获取到真实的绑定状态
