...
class OTAHandler(BaseHandler):
    ...
    async def handle_post(self, request):
        ...
        """
        try:
            remote = getattr(request.transport, "get_extra_info", lambda _: None)("peername")
            remote_str = str(remote) if remote else "unknown"
            self.logger.bind(tag=TAG).info(
                f"[小智] OTA POST 请求到达 from={remote_str} device-id={request.headers.get('device-id', '')} client-id={request.headers.get('client-id', '')}"
            )
    ...
    async def handle_get(self, request):
        ...
        try:
            remote = getattr(request.transport, "get_extra_info", lambda _: None)("peername")
            remote_str = str(remote) if remote else "unknown"
            self.logger.bind(tag=TAG).info(f"[小智] OTA GET 请求到达 from={remote_str}")
            server_config = self.config["server"]
            local_ip = get_local_ip()
